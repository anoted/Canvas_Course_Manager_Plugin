...
instead (no auth on the endpoint — bind to localhost only).

Canvas credentials come from the environment or a .env file next to
this script (client-provided environment variables win over .env):
  ...
  MCP_HOST          default 127.0.0.1 (HTTP transport only)
  MCP_PORT          default 8017 (HTTP transport only)

Run:  python canvas_mcp_server.py
HTTP endpoint (when MCP_TRANSPORT=streamable-http):  http://127.0.0.1:8017/mcp
"""
...
# Load the .env that sits next to this script, regardless of the working
# directory the MCP client launches us from. Environment variables already
# set by the client config take precedence over the file.
...
if __name__ == "__main__":
    # stdio clients own stdout for protocol messages — all logging goes to stderr.
    if not (os.environ.get("CANVAS_BASE_URL") and os.environ.get("CANVAS_API_TOKEN")):
        print(
            "WARNING: CANVAS_BASE_URL / CANVAS_API_TOKEN not set — tool calls will fail "
            "until you configure them in servers/canvas/.env and restart.",
            file=sys.stderr,
        )
    transport = os.environ.get("MCP_TRANSPORT", "stdio")
    if transport == "streamable-http":
        print(f"Canvas MCP server on http://{MCP_HOST}:{MCP_PORT}/mcp", file=sys.stderr)
