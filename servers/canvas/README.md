...
An MCP server (Python SDK, **stdio** by default) that puts the Canvas API behind
tools, resources, and prompts so an LLM client (Claude Code, Claude Desktop, Cowork)
can run the day-to-day of teaching a course: set and grade homework, build quizzes,
create pages, upload files, post announcements, and manage modules.

Normally this server is launched automatically by the `canvas-course-manager`
plugin (see the repository root README). This document covers running it directly.

## Setup

1. Copy `.env.example` to `.env` (in this directory — the server finds it
   regardless of working directory) and fill in:
   - `CANVAS_BASE_URL` — e.g. `https://yourschool.instructure.com`
   - `CANVAS_API_TOKEN` — Canvas → Account → Settings → **+ New Access Token**

   Environment variables set by the MCP client config take precedence over `.env`.

2. Install dependencies and run:

   ```
   pip install -r requirements.txt
   python canvas_mcp_server.py
   ```

   By default the server speaks **stdio** (what `claude_desktop_config.json` and
   plugin `.mcp.json` entries expect). For HTTP instead:

   ```
   MCP_TRANSPORT=streamable-http python canvas_mcp_server.py
   # endpoint: http://127.0.0.1:8017/mcp   (MCP_HOST / MCP_PORT to change)
   claude mcp add --transport http canvas http://127.0.0.1:8017/mcp
   ```

> **Security note:** the HTTP endpoint has no authentication, but your Canvas
